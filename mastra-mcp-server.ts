...
export class MCPServer extends MCPServerBase {
   ...
   */
  public getStreamableHTTPTransport():
  ...
  constructor(opts: MCPServerConfig & { resources?: MCPServerResources }) {
    ...
    if (opts.resources) {
      ...
      this.registerReadResourceHandler({
        getResourcesCallback: opts.resources.getResourceContent,
      })
  ...
  private convertAgentsToTools(
    ...
    for (const agentKey in agentsConfig) {
      ...
      if (!agent || !(agent instanceof Agent)) {
        this.logger.warn(
          `Agent instance for '${agentKey}' is invalid or missing a generate function. Skipping.`,
        )
        continue
      }
      ...
      const agentToolDefinition = createTool({
        ...
        execute: async ({ context, runtimeContext }) => {
          ...
          try {
            ...
            this.logger.error(
              `Error executing agent tool '${agentToolName}' for agent '${agent.name}':`,
              error,
            )
            throw error
          }
      ...
      this.logger.info(
        `Registered agent '${agent.name}' (key: '${agentKey}') as tool: '${agentToolName}'`,
      )
    }
    return agentTools
  }
  ...
  private convertWorkflowsToTools(
    ...
    for (const workflowKey in workflowsConfig) {
      ...
      if (
        definedConvertedTools?.[workflowToolName] ||
      ...
      this.logger.info(
        `Registered workflow '${workflow.id}' (key: '${workflowKey}') as tool: '${workflowToolName}'`,
      )
    }
    return workflowTools
  }
  ...
  convertTools(
    ...
    for (const toolName of Object.keys(tools)) {
      ...
      if (!toolInstance) {
        this.logger.warn(
          `Tool instance for '${toolName}' is undefined. Skipping.`,
        )
        continue
      }
      ...
      if (typeof toolInstance.execute !== 'function') {
        this.logger.warn(
          `Tool '${toolName}' does not have a valid execute function. Skipping.`,
        )
        continue
      }
    ...
    this.logger.info(
      `Total defined tools registered: ${Object.keys(definedConvertedTools).length}`,
    )

    const agentDerivedTools = this.convertAgentsToTools(
      agentsConfig,
      definedConvertedTools,
    )
    const workflowDerivedTools = this.convertWorkflowsToTools(
      workflowsConfig,
      definedConvertedTools,
    )
  ...
  private registerCallToolHandler() {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        ...
        if (!tool) {
          this.logger.warn(
            `CallTool: Unknown tool '${request.params.name}' requested.`,
          )
          return {
            content: [
        ...
        this.logger.debug(
          `CallTool: Invoking '${request.params.name}' with arguments:`,
          request.params.arguments,
        )

        const validation = tool.parameters.validate?.(
          request.params.arguments ?? {},
        )
        if (validation && !validation.success) {
          this.logger.warn(
            `CallTool: Invalid tool arguments for '${request.params.name}'`,
            {
        ...
        if (!tool.execute) {
          this.logger.warn(
            `CallTool: Tool '${request.params.name}' does not have an execute function.`,
          )
          return {
            content: [
              {
        ...
        this.logger.info(
          `Tool '${request.params.name}' executed successfully in ${duration}ms.`,
        )
        return {
          content: [
            {
  ...
  private registerListResourcesHandler() {
    ...
    if (!capturedResourceOptions?.listResources) {
      this.logger.warn(
        'ListResources capability not supported by server configuration.',
      )
      return
    }
    ...
    this.server.setRequestHandler(ListResourcesRequestSchema, async () => {
      ...
      if (this.definedResources) {
        ...
        try {
          ...
          this.logger.debug(
            `Fetched and cached ${this.definedResources.length} resources.`,
          )
          return { resources: this.definedResources }
        } catch (error) {
  ...
  private registerReadResourceHandler({
    ...
    this.server.setRequestHandler(
      ...
      async (request) => {
        ...
        if (!resource) {
          this.logger.warn(
            `ReadResource: Unknown resource URI '${uri}' requested.`,
          )
        ...
        try {
          ...
          const resourcesContent = Array.isArray(resourcesOrResourceContent)
            ...
            : [resourcesOrResourceContent]
          const contents: ResourceContents[] = resourcesContent.map(
            (resourceContent) => {
          ...
          const duration = Date.now() - startTime
          this.logger.info(
            `Resource '${uri}' read successfully in ${duration}ms.`,
          )
          return {
            contents,
          }
          ...
          const duration = Date.now() - startTime
          this.logger.error(
            `Failed to get content for resource URI '${uri}' in ${duration}ms`,
            { error },
          )
          throw error
        }
  ...
  private registerListResourceTemplatesHandler() {
    ...
    if (
      ...
      typeof this.resourceOptions.resourceTemplates !== 'function'
    ) {
      this.logger.warn(
      ...
      this.server.setRequestHandler(
        ...
        async () => {
          this.logger.debug(
            'Handling ListResourceTemplates request (no templates configured or resourceOptions issue)',
          )
      ...
      async () => {
        ...
        if (this.definedResourceTemplates) {
          ...
          try {
            ...
            this.logger.debug(
              `Fetched and cached ${this.definedResourceTemplates.length} resource templates.`,
            )
            return { resourceTemplates: this.definedResourceTemplates }
          } catch (error) {
            this.logger.error(
              'Error fetching resource templates via resourceTemplates():',
              { error },
            )
            // Re-throw to let the MCP Server SDK handle formatting the error response
            throw error
  ...
  private registerSubscribeResourceHandler() {
    ...
    if (!SubscribeRequestSchema) {
      this.logger.warn(
        'SubscribeRequestSchema not available, cannot register SubscribeResource handler.',
      )
      return
    }
    this.subscribeResourceHandlerIsRegistered = true
    this.server.setRequestHandler(
      SubscribeRequestSchema as any,
  ...
  private registerUnsubscribeResourceHandler() {
    ...
    this.server.setRequestHandler(
      UnsubscribeRequestSchema as any,
      async (request: { params: { uri: string } }) => {
        const uri = request.params.uri
        this.logger.info(
          `Received resources/unsubscribe request for URI: ${uri}`,
        )
  ...
  public async startSSE({
    ...
    req,
    res,
  }: MCPServerSSEOptions): Promise<void> {
  ...
  public async startHonoSSE({
    ...
    if (url.pathname === ssePath) {
      ...
      const message = await this.sseHonoTransports
        .get(sessionId)
        ?.handlePostMessage(context)
      if (!message) {
  ...
  public getToolListInfo(): {
    ...
    this.logger.debug(
      `Getting tool list information for MCPServer '${this.name}'`,
    )
    return {
  ...
  public getToolInfo(
    ...
    if (!tool) {
      this.logger.debug(
        `Tool '${toolId}' not found on MCPServer '${this.name}'`,
      )
      return undefined
    }
    this.logger.debug(
      `Getting info for tool '${toolId}' on MCPServer '${this.name}'`,
    )
    return {
      name: tool.name,
      description: tool.description,
  ...
  public async executeTool(
    ...
    if (!tool) {
      this.logger.warn(
        `ExecuteTool: Unknown tool '${toolId}' requested on MCPServer '${this.name}'.`,
      )
    ...
    if (
      ...
      if (!validation.success) {
        const errorMessages = validation.error.errors
          ...
          .join(', ')
        this.logger.warn(
          `ExecuteTool: Invalid tool arguments for '${toolId}': ${errorMessages}`,
          {
    ...
    if (!tool.execute) {
      this.logger.error(
        `ExecuteTool: Tool '${toolId}' does not have an execute function.`,
      )
    ...
    try {
      ...
      throw error instanceof Error
        ? error
        : new Error(`Execution of tool '${toolId}' failed: ${String(error)}`)
    }
  }
}
