...
import { enhancedAgentProxyTool } from "./tools/enhancedAgentProxyTool.js";
import { getMCPServerPort, getMCPPaths } from "./config.js";

// Instantiate MCPServer with enhanced tools
const mcpServerInstance = new MCPServer({
  name: "EnhancedMastraAgentProxyMCPServer",
  version: "1.0.0",
  tools: {
    callMastraAgent: enhancedAgentProxyTool, // Enhanced tool with server:agentId support
    listMastraAgents: listMastraAgentsTool, // Unified multi-server agent listing with conflict detection
  },
});
...
if (useStdio) {
  ...
  httpServer.listen(PORT, () => {
    console.log(`Enhanced MCP Server with HTTP/SSE transport listening on port ${PORT}`);
  ...
  process.on('SIGINT', () => {
    ...
    httpServer.close(() => {
      console.log('Enhanced MCP server shut down complete.');
      process.exit(0);
    });
  });
