...
async function testAgentProxyMCPServer() {
  const mcpClient = new MCPClient({
    servers: {
      mastraProxy: {
        // An arbitrary name for this client's connection config
        // Configuration for HTTP/SSE transport
        url: new URL(
          `http://localhost:${process.env.MCP_SERVER_PORT || 3001}${process.env.MCP_SSE_PATH || '/mcp/sse'}`,
        ),
      },
    },
  })
  ...
  try {
    ...
    console.log(
      'Available MCP Tools from mastraProxy:',
      proxyTools.map((t: { id: string }) => t.id),
    )

    // Test listMastraAgents tool if available
    const listAgentsTool = proxyTools.find(
      (t: { id: string }) => t.id === 'listMastraAgents',
    )
    if (listAgentsTool) {
    ...
    const callAgentTool = proxyTools.find(
      (t: { id: string }) => t.id === 'callMastraAgent',
    )
    if (callAgentTool) {
